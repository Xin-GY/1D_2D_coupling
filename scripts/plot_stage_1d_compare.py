...
def main(root: Path | str = Path('artifacts') / 'coupling_sweep') -> None:
    ...
    for case_name in scheduler_case_names(summary_rows):
        rows = case_rows(root, case_name, 'stage_timeseries_1d.csv')
        x, y = series_from_rows(rows, 'time', 'stage', filter_key='control_id', filter_value='mainstem_mid')
        ax.plot(x, y, label=case_name.split('_', 3)[-1])
    ax.set_title('1D Stage Comparison')
    ax.set_xlabel('Time (s)')
    ax.set_ylabel('Stage (m)')
    ax.grid(True, alpha=0.3)
    ax.legend(fontsize=8)
    save_figure(fig, ensure_plot_dir(root) / 'stage_1d_compare.png')
