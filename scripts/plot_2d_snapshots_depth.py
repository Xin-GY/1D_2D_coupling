...
def main(root: Path | str | None = None) -> None:
    ...
    for case_name, rows in rows_by_case.items():
        ...
        assert_required_columns(rows, ('snapshot_id', 'cell_id', 'depth'), f'two_d_snapshots for {case_name}')
        arrays.append(
            build_snapshot_value_array(
                rows,
                'snapshot_3',
    ...
    fig.suptitle('典型时刻二维水深分布对比', fontsize=14)
    fig.colorbar(sc, ax=axes.ravel().tolist(), label='水深 (m)')
